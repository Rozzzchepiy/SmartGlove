...
for file in os.listdir(data_dir):
    if file.endswith(".csv"):
        ...
        if df.shape == (75, 39):
            ...
            print(f"⚠️ Пропускаю {file} (очікувалось 75x39, отримано {df.shape})")
...
# === 3️⃣ Поділ train/test ===
X_train, X_test, y_train, y_test = train_test_split(
    samples, y, test_size=0.2, random_state=42, stratify=y
)

# === 4️⃣ Масштабування (scaling) ===
# Створюємо scaler і навчаємо його ТІЛЬКИ на train
scaler = MinMaxScaler()

N_train, T, F = X_train.shape
N_test = X_test.shape[0]

# Перетворюємо у 2D, щоб scaler міг навчитися
X_train_2d = X_train.reshape(-1, F)
X_test_2d = X_test.reshape(-1, F)

scaler.fit(X_train_2d)  # тільки на тренувальних даних
